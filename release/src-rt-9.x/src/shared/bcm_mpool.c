...
int BCMATTACHFN(bcm_mpm_init)(osl_t *osh, int max_pools, bcm_mpm_mgr_h *mgrp)
{
	...
	mgr = (bcm_mpm_mgr_h) MALLOCZ_PERSIST(osh, (sizeof(*mgr)));
	if (mgr == NULL) {
		return (BCME_NOMEM);
	}
	...
	pool_objs = (bcm_mp_pool_t *) MALLOCZ_PERSIST(osh, max_pools * sizeof(bcm_mp_pool_t));
	if (pool_objs == NULL) {
		MFREE_PERSIST(osh, mgr, sizeof(*mgr));
		return (BCME_NOMEM);
	}
...
int BCMATTACHFN(bcm_mpm_deinit)(bcm_mpm_mgr_h *mgrp)
{
	...
	MFREE_PERSIST(osh, mgr->pool_objs, mgr->max_pools * sizeof(bcm_mp_pool_t));
                                    ...
                                    bcm_mp_pool_h *newp)
{
	...
	if ((type == BCM_MP_TYPE_PREALLOC) && (memstart == NULL)) {
		memstart = MALLOC_PERSIST(mgr->osh, padded_obj_sz * nobj);
		if (memstart == NULL) {
