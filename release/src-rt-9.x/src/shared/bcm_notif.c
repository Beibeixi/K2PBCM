...
#ifdef EVENT_LOG_COMPILE
	event_log_tag_start(EVENT_LOG_TAG_NOTIF_ERROR, EVENT_LOG_SET_ERROR, EVENT_LOG_TAG_FLAG_LOG);
#endif
	/* Allocate global notifier module state. */
	if ((notif_module = MALLOCZ_PERSIST(osh, sizeof(*notif_module))) == NULL) {
...
static void BCMATTACHFN(dealloc_module)(bcm_notif_module_t *notif_module)
{
	if (notif_module != NULL) {
		...
		MFREE_PERSIST(notif_module->osh, notif_module, sizeof(*notif_module));
	}
}
