...
#else
			*trx_size = ROUNDUP(le32_to_cpu(trx->len), mtd->erasesize);
#endif
			if (trx->offsets[1] == 0)
				continue;
			/*
...
#endif
		bcm947xx_flash_parts[nparts].size = trx_size - (rfs_off - vmlz_off);
		bcm947xx_flash_parts[nparts].offset = rfs_off;
		bcm947xx_flash_parts[nparts].mask_flags = MTD_WRITEABLE; /* forces on read only */
		offset = bcm947xx_flash_parts[nparts].offset + bcm947xx_flash_parts[nparts].size;
		nparts++;

		/*** Setup jffs2 MTD partition ***/
		bcm947xx_flash_parts[nparts].name = "jffs2";
		bcm947xx_flash_parts[nparts].size = knl_size - trx_size;
		bcm947xx_flash_parts[nparts].offset = trx_size + vmlz_off;
		offset = bcm947xx_flash_parts[nparts].offset + bcm947xx_flash_parts[nparts].size;
		nparts++;
